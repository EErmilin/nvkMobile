...
export const BlogerContent_Photo: FC<TProps> = ({ post, onPress: onPressInner }) => {
...
export const BlogerContent_Video: FC<TProps> = ({ post, onPress: onPressInner }) => {
  const onPress = () => onPressInner(post);
  return (
    <TouchableOpacity onPress={onPress}>
      <FastImage
      ...
      />
      <View
        style={[styles.vidoe_image, { position: 'absolute', backgroundColor: "#000", opacity: 0.3 }]}
      />
...
export const BlogerContent_Audio: FC<TProps> = ({ post, onPress: onPressInner }) => {
  const onPress = () => onPressInner(post);
  return (
    <TouchableOpacity onPress={onPress}>
      <FastImage
      ...
      />
      <View
        style={[styles.audio_image, { position: 'absolute', backgroundColor: "#000", opacity: 0.3 }]}
      />
...
const ContentViewed = ({ count }: { count: number }) => <View style={styles.viewers_wrapper}>
  <BlogerProfile_VideoPlay_icon />
  <Text style={styles.viewers_text}>{`${count}`}</Text>
</View>

const ContentListened = ({ count }: { count: number }) => <View style={styles.viewers_wrapper}>
