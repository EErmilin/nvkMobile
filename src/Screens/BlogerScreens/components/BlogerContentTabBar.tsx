...
const BlogerContentTabBar: React.FC<BlogerContentTabBarProps> = ({ tabs, activeTab, goToPage }) => {
    ...
    return (
        <View style={{ flexDirection: 'row', backgroundColor: '#fff' }}>
            {tabs.map((tab, index) => (
                ...
                >
                    {tab === 'photo' && <BlogerProfile_Photo_icon active={activeTab === index} />}
                    {tab === 'video' && <BlogerProfile_Video_icon active={activeTab === index} />}
                    {tab === 'audio' && <BlogerProfile_Audio_icon active={activeTab === index} />}
